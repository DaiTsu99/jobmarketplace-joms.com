<x-layout>
    <header class="flex justify-between border border-gray-200 p-6 rounded-xl max-w-xxl mx-auto mt-20 text-center ring-offset-2 ring-offset-gray-100 shadow-md">
        ...
        <div class="text-left">
            <h2 class="text-xl font-semibold">Edit Job Post</h2>
            <h3 class="text-sm font-light italic">Update your job post details and submit</h3>
        </div>
            ...
            <div class="-my-2 overflow-x-auto sm:-mx-6 lg:-mx-8">
                <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                    <div class="shadow overflow-hidden border-b p-4 border-gray-200 sm:rounded-lg">
                        <form action="/myJobs/{{$job->id}}/update" method="POST" onsubmit="return confirm('Are you sure you have filled correctly? Please ensure correct spellings especially for tags')">
                            @csrf
                            <div class="mb-6">
                            ...
                            <div class="mb-6">
                                ...
                                name="location"
                                id="location"
                                value="{{old('location', $job->location->name.", ".$job->location->state->name)}}"
                                required
                                >
                            ...
                            <div class="mb-6">
                                ...
                                <textarea class="border border-gray-400 p-2 w-full "
                                    ...
                                    id="excerpt"
                                    required
                                >{{strip_tags(old('excerpt',$job->excerpt))}}</textarea>
                            ...
                            <div class="mb-6">
                                ...
                                <textarea class="border border-gray-400 p-2 w-full "
                                    ...
                                    id="description"
                                    required
                                >{{strip_tags(old('description', $job->description))}}</textarea>
                            ...
                            <div class="mb-6">
                                ...
                                <textarea class="border border-gray-400 p-2 w-full "
                                    ...
                                    id="skillNeeded"
                                    required
                                >{{strip_tags(old('skillNeeded', $job->skillNeeded))}}</textarea>
                            ...
                            <div class="mb-6">
                                ...
                                <textarea class="border border-gray-400 p-2 w-full "
                                    ...
                                    id="jobScope"
                                    required
                                >{{strip_tags(old('jobScope', $job->jobScope))}}</textarea>
                                            ...
                                            @if($tag->category->name =="Job Type")
                                                <label class="block text-left px-3 mb-2 text-sm text-gray-700">
                                                    <input type="checkbox" name="jobType[]" class="mx-2" value="{{$tag->name}}"
                                                           @if(is_array(old('jobType')) && in_array($tag->name, old('jobType'))) checked
                                                           @else

                                                               @foreach($jobtags as $jobtag) {{-- for each tags associated with job in database--}}

                                                                   @if($jobtag->name == $tag->name)
                                                                       checked
                                                                       @endif
                                                               @endforeach
                                                        @endif>
                                            ...
                                            @if($tag->category->name =="Job Experience")
                                                <label class="block text-left px-3 mb-2 text-sm text-gray-700">
                                                    <input type="checkbox" name="jobExperience[]" class="mx-2" value="{{$tag->name}}"
                                                        @if(is_array(old('jobExperience')) && in_array($tag->name, old('jobExperience')))
                                                              checked
                                                        @else
                            ...
                            <div class="mb-6">
                                ...
                                <div class="italic text-sm ml-4 mb-2">Enter the sector(s) separated by comma, do follow the spelling, and you may also add a new tag if it's not available</div>
                                <x-panel>
                                <input class="border border-gray-400 p-2 w-full rounded"
                                type="text"
                                name="sector"
                                ...
                                <div class="mt-2">
                                    ...
                                    @foreach($jobtags as $jobtag)
                                        @if($jobtag->category->name == "Sector")
                                            <script>$('#sector').addTag({!! json_encode($jobtag->name) !!})</script>
                                        @endif
                                    @endforeach
                                @error('sector')<span class="text-xs text-red-500 mt-2">{{ $message }}</span>@enderror
                                </x-panel>
                            </div>
                            <x-form.button>
                                Update
