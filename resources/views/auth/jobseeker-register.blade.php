<x-layout>
    <x-auth-card>
        ...
        <x-slot name="navi">
            <a href="{{ route('registerJobseeker') }}" class="mt-3 p-3 bg-blue-200 font-semibold rounded-md float-left">Jobseeker Registration</a>
            <a href="{{ route('registerEmployer') }}" class="mt-3 p-3 bg-white rounded-md float-left">Employer Registration</a>
        </x-slot>

        <!-- Validation Errors -->
        <x-auth-validation-errors class="mb-4" :errors="$errors" />

        <form method="POST" action="{{ route('registerJobseeker') }}">
            @csrf
